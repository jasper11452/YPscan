...
插件内 Runner 使用专用持久 Chrome 打开固定蒲公英达人广场。报价类型为“图文笔记”或“视频笔记”，与“笔记类型”的内容形式筛选相互独立；例如“视频笔记为主”可以同时按图文报价筛选。列表“全部报价/¥xx起”只作起始价展示，只有结构化列表字段或详情页一口价可验证目标报价。随后依次尝试完整筛选、仅关键词、无筛选达人广场和通用可见 DOM 召回。Agent 不操作页面，不传快照，不触发平台原生导出。
