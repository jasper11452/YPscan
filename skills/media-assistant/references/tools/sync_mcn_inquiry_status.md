...
Call when the user asks to retrieve institutional responses after `create_with_distributions`. Pass the exact `requirement_id`, `project_id`, and complete `supplierIds` from the current inquiry flow. Do not mix requirements or add suppliers that were not part of that send.
